...
//	TODO: Add Noise gate
    ...
    public static final int REQ_CODE_COVER_AUDIO = 12;
    ...
    private ImageButton btSelectSound;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btSelectSound = findViewById(R.id.btSelectSound);
        ...
        btSelectSound.setOnClickListener(this);
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.btn_play) {
            ...
            if (checkStoragePermissionImport()) {
                startFileSelector(REQ_CODE_IMPORT_AUDIO);
            }
        } else if (id == R.id.txt_name) {
            presenter.onRenameRecordClick();
        } else if (id == R.id.btSelectSound) {
            if (presenter.getCoverPath() == null) {
                if (checkStoragePermissionImport()) {
                    startFileSelector(REQ_CODE_COVER_AUDIO);
                }
                return;
            }
            presenter.setCoverPath(null);
            tvLyric.setText("");
            btSelectSound.setImageResource(R.drawable.ic_audiotrack);
        }
    }

    private void startFileSelector(int requestCode) {
...
//		intent_upload.setAction(Intent.ACTION_GET_CONTENT);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        } else if (requestCode == REQ_CODE_COVER_AUDIO && resultCode == RESULT_OK) {
            try {
                String path = FileUtil.getPath(getApplicationContext(), data.getData());
                if (path == null) return;

                presenter.setCoverPath(path);
                btSelectSound.setImageResource(R.drawable.ic_delete);

                MP3File mp3file = (MP3File) AudioFileIO.read(new File(path));
                if (mp3file.hasID3v2Tag()) {
                    tvLyric.setText(mp3file.getID3v2Tag().getFirst(FieldKey.LYRICS));
                }
            } catch (URISyntaxException | IOException | CannotReadException |
